...
| `~/.hermes/sessions/` | Migas de pan de solicitudes, transcripciones `*.jsonl` del gateway y (opcionalmente) snapshots JSON por sesión cuando `sessions.write_json_snapshots: true` está configurado. El enrutamiento del gateway vive en `state.db` `gateway_routing`; los snapshots por sesión están desactivados por defecto. |
