    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        if _discord_tools_loaded():
            ...
            if src.scope_id:
                id_lines.append(f"  - Guild: `{src.scope_id}`")
...
# written to the routing store.  On rehydration after a gateway restart the
...
class SessionEntry:
    ...
    # agent).  Persisted to state.db gateway_routing so the flag survives gateway
...
class SessionStore:
    ...
    def _routing_scope(self) -> str:
        ...
        The resolved sessions_dir path namespaces routing entries so two stores
        with different directories (tests, multi-profile setups sharing one
    ...
    def _ensure_loaded_locked(self) -> None:
        ...
        The ``gateway_routing`` table in state.db is the sole durable routing
        store.  Obsolete sessions.json mirrors are ignored.
        """
        if self._loaded:
            return
    ...
    def _legacy_slack_session_key(self, source: SessionSource) -> Optional[str]:
        ...
        if source.platform != Platform.SLACK or not source.scope_id:
            return None
        legacy_source = replace(source, scope_id=None)
        return build_session_key(
            legacy_source,
    ...
    def _recovered_row_matches_source_scope(
        ...
        if not isinstance(origin, dict):
            return False
        return origin.get("scope_id") == source.scope_id
    ...
    def set_expiry_finalized(
        ...
        state.db flag in sync with the routing index so the flag survives
        routing-entry pruning/loss.
    ...
    def _is_session_ended_in_db(self, session_id: str) -> bool:
        ...
        the row without clearing the routing entry) would otherwise be reused as a
            ...
            if session_key in self._entries and not force_new:
                ...
                if _is_stale and entry.session_id == _stale_session_id:
                    ...
                    logger.warning(
                        ...
                        "state.db but still live in gateway routing; dropping "
    ...
    def set_session_metadata(
        ...
        the routing index (state.db gateway_routing table) so they survive
        gateway restarts.
        """
        with self._lock:
            self._ensure_loaded_locked()
