...
class _GatewayConfigBase(BaseModel):
...
class HomeChannel(_GatewayConfigBase):
    """
...
class SessionResetPolicy(_GatewayConfigBase):
    """
    ...
    @classmethod
    def _coerce_notify(cls, value: Any) -> Any:
...
class ChannelOverride(_GatewayConfigBase):
    """
...
class PlatformConfig(_GatewayConfigBase):
    ...
    def _coerce_mapping(cls, data: Any) -> Any:
        ...
        if isinstance(data.get("home_channel"), dict):
            data = dict(data)
            data["home_channel"] = HomeChannel.model_validate(data["home_channel"])
        ...
        if isinstance(raw_overrides, dict):
            ...
            data["channel_overrides"] = {
                str(cid): (
                    ...
                    else ChannelOverride.model_validate(ov_data)
                )
                for cid, ov_data in raw_overrides.items()
                if isinstance(ov_data, (dict, ChannelOverride))
            }
        return data
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PlatformConfig":
        ...
        if "home_channel" in normalized and not isinstance(
            normalized["home_channel"], (dict, HomeChannel)
        ):
            normalized.pop("home_channel", None)
...
class StreamingConfig(_GatewayConfigBase):
...
class GatewayConfig(_GatewayConfigBase):
    """
    ...
    # Storage paths
    sessions_dir: Path = Field(default_factory=lambda: _hermes_home() / "sessions")
    ...
    profile_routes: List[ProfileRoute] = Field(default_factory=list)
    ...
    def _normalize_input(cls, value: Any) -> Any:
        ...
        for platform_name, platform_data in _coerce_dict(data.get("platforms", {})).items():
            if isinstance(platform_data, PlatformConfig):
                try:
                    platforms[Platform(platform_name)] = platform_data
                except ValueError:
                    pass
                continue
        ...
        if "default_reset_policy" in data:
            data["default_reset_policy"] = SessionResetPolicy.model_validate(
                data["default_reset_policy"]
            )
        data["reset_by_type"] = {
            str(type_name): SessionResetPolicy.model_validate(policy_data)
        ...
        for platform_name, policy_data in _coerce_dict(data.get("reset_by_platform", {})).items():
            try:
                reset_by_platform[Platform(platform_name)] = SessionResetPolicy.model_validate(policy_data)
            except ValueError:
                pass
        data["reset_by_platform"] = reset_by_platform

        if "sessions_dir" in data:
            data["sessions_dir"] = Path(data["sessions_dir"])

        if "stt_enabled" not in data and isinstance(data.get("stt"), dict):
            data["stt_enabled"] = data["stt"].get("enabled")
        if "stt_echo_transcripts" not in data and isinstance(data.get("stt"), dict):
            data["stt_echo_transcripts"] = data["stt"].get("echo_transcripts")

        if "systemd_watchdog_seconds" not in data:
            data["systemd_watchdog_seconds"] = nested_gateway.get("systemd_watchdog_seconds", 0)
        if "loop_watchdog" not in data:
            data["loop_watchdog"] = nested_gateway.get("loop_watchdog", True)
        if "multiplex_profiles" not in data:
            data["multiplex_profiles"] = nested_gateway.get("multiplex_profiles", False)
        if "max_concurrent_sessions" not in data:
            data["max_concurrent_sessions"] = nested_gateway.get("max_concurrent_sessions")

        env_multiplex = _env_multiplex_profiles_override()
        if env_multiplex is not None:
            data["multiplex_profiles"] = env_multiplex

        data["unauthorized_dm_behavior"] = _normalize_unauthorized_dm_behavior(
            data.get("unauthorized_dm_behavior"),
            "pair",
        )
        data["streaming"] = StreamingConfig.from_dict(data.get("streaming", {}))
        return data
    ...
    @classmethod
    def _coerce_true_default_bools(cls, value: Any) -> Any:
        return _coerce_bool(value, True)

    @field_validator("thread_sessions_per_user", "multiplex_profiles", mode="before")
    @classmethod
    def _coerce_false_default_bools(cls, value: Any) -> Any:
        return _coerce_bool(value, False)

    @field_validator("systemd_watchdog_seconds", mode="before")
    @classmethod
    def _coerce_systemd_watchdog(cls, value: Any) -> Any:
        return coerce_systemd_watchdog_seconds(value)

    @field_validator("max_concurrent_sessions", mode="before")
    @classmethod
    def _coerce_max_concurrent_sessions(cls, value: Any) -> Any:
        return _coerce_optional_positive_int(value, "gateway.max_concurrent_sessions")

    @field_validator("session_store_max_age_days", mode="before")
    @classmethod
    def _coerce_session_store_max_age(cls, value: Any) -> Any:
        try:
            return max(int(value), 0)
        except (TypeError, ValueError):
            return 90

    @field_validator("quick_commands", mode="before")
    @classmethod
    def _coerce_quick_commands(cls, value: Any) -> Any:
        return value if isinstance(value, dict) else {}
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        from hermes_cli.config import _load_config_data_for_typed_model
    ...
    if yaml_cfg:
            ...
            if isinstance(gateway_section, dict):
                ...
                if "loop_watchdog" in gateway_section:
            ...
            # ``_apply_env_overrides()`` after ``GatewayConfig`` validation.
            if _pr is not None:
    ...
    config = GatewayConfig.model_validate(gw_data)
