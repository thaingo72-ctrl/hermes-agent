...
def fake_mcp_server(populated_sessions_dir, mock_session_db, monkeypatch):
    ...
    monkeypatch.setattr(mcp_serve, "_load_sessions_index", lambda: json.loads(
        (populated_sessions_dir / "sessions.json").read_text(encoding="utf-8")
    ))
...
class TestEdgeCases:
    ...
    def test_sessions_without_origin(self, sessions_dir, monkeypatch):
        ...
        import mcp_serve
        monkeypatch.setattr(mcp_serve, "_load_sessions_index", lambda: data)
...
class TestEventBridgePollE2E:
    ...
    def test_poll_detects_new_messages(self, tmp_path, monkeypatch):
        """Write to SQLite, verify EventBridge picks it up."""
        import mcp_serve
        ...
        # Stale sessions.json fixture: the bridge should read routing from SQLite.
        sessions_data = {
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        its first message land in the same file. This test asserts the index
        is refreshed on a db-mtime bump, so a conversation the bridge has never
        seen before is emitted on the same tick.
        """
        import mcp_serve
        ...
        # The routing index now comes from _load_sessions_index() backed by
        # state.db. Stub it to return the new conversation, simulating the
        # gateway having just written the session row + first message in one
