...
def test_deactivated_request_guard_suppresses_late_callbacks(monkeypatch):
    ...
    agent.interim_assistant_callback = object()
    agent.show_commentary = True
    agent._fire_streamed_codex_commentary = lambda text: emitted_commentary.append(text)
    ...
    def fake_consume(
        ...
        *,
        ...
        on_commentary_message("late commentary")
        on_first_delta()
...
def test_ttfb_watchdog_error_wins_over_obsolete_worker_interrupt(tmp_path, monkeypatch):
    """A guard-aware worker that exits promptly after timeout must not turn the
    watchdog's retryable timeout into a user-interrupt signal."""
    from agent import chat_completion_helpers as h
    ...
    def guard_aware_hang(
        api_kwargs, client=None, on_first_delta=None, request_guard=None
    ):
        assert request_guard is not None
        while request_guard.is_active():
            time.sleep(0.01)
        raise InterruptedError("obsolete after watchdog")

    monkeypatch.setattr(agent, "_run_codex_stream", guard_aware_hang)

    with pytest.raises(TimeoutError, match="TTFB threshold"):
        h.interruptible_api_call(agent, {"model": "gpt-5.6-sol", "input": "hi"})


def test_hard_timeout_remains_effective_while_stream_cleanup_blocks(
    tmp_path, monkeypatch
):
    """A terminal event is not complete until stream/client cleanup finishes;
    blocked cleanup must remain cancellable by the absolute timeout."""
    from agent import chat_completion_helpers as h

    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("HERMES_CODEX_TTFB_TIMEOUT_SECONDS", "0")
    monkeypatch.setenv("HERMES_CODEX_EVENT_STALE_TIMEOUT_SECONDS", "0")
    monkeypatch.setenv("HERMES_CODEX_HARD_TIMEOUT_SECONDS", "0.2")
    ...
    dummy_client = SimpleNamespace(
        responses=SimpleNamespace(create=lambda **kwargs: BlockingCloseStream())
    )
    monkeypatch.setattr(agent, "_create_request_openai_client", lambda **k: dummy_client)
    monkeypatch.setattr(agent, "_abort_request_openai_client", lambda *a, **k: None)
    monkeypatch.setattr(agent, "_close_request_openai_client", lambda *a, **k: None)
    ...
    try:
        with pytest.raises(TimeoutError, match="timed out"):
            h.interruptible_api_call(
                agent, {"model": "gpt-5.6-sol", "input": "cleanup race"}
            )
...
def test_explicit_large_request_size_gate_disables_ttfb_instead_of_extending_it(
    tmp_path, monkeypatch
):
    """The opt-in size gate disables TTFB entirely, rather than swapping in a
    longer timeout that can later be recapped and kill a healthy slow start."""
    from agent import chat_completion_helpers as h

    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("HERMES_CODEX_TTFB_TIMEOUT_SECONDS", "0.5")
    monkeypatch.setenv("HERMES_CODEX_TTFB_DISABLE_ABOVE_TOKENS", "100")
    monkeypatch.setenv("HERMES_CODEX_TTFB_MAX_SECONDS", "0.5")
    ...
    def fake_stream(api_kwargs, client=None, on_first_delta=None, request_guard=None):
        # Longer than both the configured TTFB and max cap. If the size gate
        # merely extends/reassigns TTFB, this request is killed before returning.
        time.sleep(1.2)
        return sentinel

    monkeypatch.setattr(agent, "_run_codex_stream", fake_stream)

    large_input = "x" * 1_000  # ~250 estimated tokens, above the explicit gate.
