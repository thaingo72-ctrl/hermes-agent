...
from tui_gateway import methods_session_state, server
...
def test_legacy_session_rebinding_files_are_removed():
    root = Path(__file__).resolve().parents[2]
    assert not (root / "tui_gateway" / "methods_session.py").exists()
    assert not (root / "tui_gateway" / "method_ctx.py").exists()
