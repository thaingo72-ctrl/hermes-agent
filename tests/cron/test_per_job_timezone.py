...
def test_due_check_compares_absolute_instants_across_dst_fold(tmp_path, monkeypatch):
    monkeypatch.setattr("cron.jobs.CRON_DIR", tmp_path / "cron")
    monkeypatch.setattr("cron.jobs.JOBS_FILE", tmp_path / "cron/jobs.json")
    monkeypatch.setattr("cron.jobs.OUTPUT_DIR", tmp_path / "cron/output")
    ...
    job = create_job(prompt="dst-fold", schedule="TZ=UTC 30 5 * * *")
    jobs = load_jobs()
    jobs[0]["next_run_at"] = "2026-11-01T05:30:00+00:00"
    save_jobs(jobs)
