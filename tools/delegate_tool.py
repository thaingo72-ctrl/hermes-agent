...
def _run_single_child(
    ...
    **_kwargs,
    ...
    try:
        ...
        # Children share the parent's backend but own their cwd record. Seed
        # from the parent so they start in the same directory while later `cd`s
        # stay isolated to the child task key.
        try:
            from agent.runtime_cwd import get_recorded_session_cwd, record_session_cwd

            record_session_cwd(child_task_id, get_recorded_session_cwd(parent_task_id))
        except Exception as e:
