...
The MCP server reads conversation data and gateway routing directly from Hermes's session store in `~/.hermes/state.db`. A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `hermes send` CLI.
