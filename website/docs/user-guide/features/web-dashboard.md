...
The default `hermes-agent` install ships the PTY helper, but not the HTTP stack. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). Install it with:

```bash
cd ~/.hermes/hermes-agent && uv pip install -e ".[web]"
```

The `web` extra pulls in FastAPI/Uvicorn. `cd ~/.hermes/hermes-agent && uv pip install -e ".[all]"` includes it and is the easiest path if you also want messaging/voice/etc.
...
- `ptyprocess` — installed by the core package
