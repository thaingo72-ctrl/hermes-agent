...
def _terminal_task_cwd(session: dict | None) -> str:
    ...
    The completion module validates local paths on the host so file completion
    does not point at nonsense. Non-local terminal backends are different: their cwd is
...
def _session_cwd(session: dict | None) -> str:
    ...
    return _methods_complete._completion_cwd(None, _completion_rpc_services)
...
def _pet_sig() -> tuple:
    ...
    if not pet_cfg or not pet_cfg.get("enabled"):
        return ("off",)
    try:
        enabled, pet, scale = methods_pet.pet_active_selection(_pet_services)
        if not enabled or pet is None or not pet.exists:
            return ("off",)
        return (pet.slug, methods_pet.pet_sheet_revision(pet.spritesheet), scale)
...
def _pet_changed_payload() -> dict:
    ...
    try:
        ...
        return {
            ...
            "scale": scale,
            "spritesheetRevision": methods_pet.pet_sheet_revision(pet.spritesheet),
        }
    except Exception:  # noqa: BLE001 - cosmetic, never break the watcher
        return {"enabled": False}
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "attached_images": [],
            "image_counter": 0,
            "cwd": cwd or _methods_complete._completion_cwd(None, _completion_rpc_services),
            "cols": cols,
            "slash_worker": None,
...
# All split domains register direct service-injected callables.
...
_completion_rpc_services = _methods_complete.CompletionServices(
    ...
    load_cfg=_load_cfg,
    apply_managed=_apply_managed,
    resolve_model=_resolve_model,
)
_methods_complete.register(
    _methods,
    services=_completion_rpc_services,
)
_config_rpc_services = _methods_config.ConfigServices(
    ok=_ok,
    err=_err,
    ...
    detail_section_names=_DETAIL_SECTION_NAMES,
    load_cfg=lambda: _load_cfg(),
    load_cfg_raw=lambda: _load_cfg_raw(),
    save_cfg=lambda cfg: _save_cfg(cfg),
    write_config_key=lambda key, value: _write_config_key(key, value),
    compute_mcp_rev=lambda: _methods_config.compute_mcp_rev(_load_cfg),
    get_db=lambda: _get_db(),
    completion_cwd=lambda params=None: _methods_complete._completion_cwd(
        params,
        _completion_rpc_services,
    ),
    git_branch_for_cwd=lambda cwd: _git_branch_for_cwd(cwd),
    git_common_repo_root_for_cwd=lambda cwd: _git_common_repo_root_for_cwd(cwd),
    resolve_cwd_git=lambda cwd: _resolve_cwd_git(cwd),
    resolve_model=lambda: _resolve_model(),
    load_service_tier=lambda: _load_service_tier(),
    load_busy_input_mode=lambda: _load_busy_input_mode(),
    load_approval_mode=lambda: _load_approval_mode(),
    load_tool_progress_mode=lambda: _load_tool_progress_mode(),
    coerce_statusbar=lambda raw: _coerce_statusbar(raw),
    display_mouse_tracking=lambda display: _display_mouse_tracking(display),
    apply_model_switch=lambda *a, **kw: _apply_model_switch(*a, **kw),
    start_agent_build=lambda sid, session: _start_agent_build(sid, session),
    wait_agent=lambda session, rid: _wait_agent(session, rid),
    persist_live_session_runtime=lambda session: _persist_live_session_runtime(session),
    session_info=lambda *a, **kw: _session_info(*a, **kw),
    emit=lambda event, sid, payload: _emit(event, sid, payload),
    broadcast_global_event=lambda event, payload=None: _broadcast_global_event(event, payload),
    resolve_skin=lambda: resolve_skin(),
    note_skin_broadcast=lambda: _note_skin_broadcast(),
    validate_personality=lambda value, cfg=None: _validate_personality(value, cfg),
    apply_personality_to_session=lambda sid, session, prompt, name: _apply_personality_to_session(
        sid, session, prompt, name
    ),
)
_methods_config.register(_methods, services=_config_rpc_services)
...
_prompt_submit_rpc_services = _methods_prompt.PromptSubmitServices(
    sess_nowait=lambda params, rid: _sess_nowait(params, rid),
    ok=lambda rid, result: _ok(rid, result),
    err=lambda rid, code, msg: _err(rid, code, msg),
...
_attachment_rpc_services = _methods_prompt.AttachmentServices(
    sess=lambda params, rid: _sess(params, rid),
    sess_nowait=lambda params, rid: _sess_nowait(params, rid),
    ok=lambda rid, result: _ok(rid, result),
    err=lambda rid, code, msg: _err(rid, code, msg),
...
_background_rpc_services = _methods_prompt.BackgroundServices(
    sess=lambda params, rid: _sess(params, rid),
    ok=lambda rid, result: _ok(rid, result),
    err=lambda rid, code, msg: _err(rid, code, msg),
    session_cwd=lambda session: _session_cwd(session),
...
_response_rpc_services = _methods_prompt.ResponseServices(
    ok=lambda rid, result: _ok(rid, result),
    err=lambda rid, code, msg: _err(rid, code, msg),
...
_approval_rpc_services = _methods_prompt.ApprovalServices(
    sess=lambda params, rid: _sess(params, rid),
    ok=lambda rid, result: _ok(rid, result),
    err=lambda rid, code, msg: _err(rid, code, msg),
)
_methods_prompt.register(
    _methods,
    prompt_submit_services=_prompt_submit_rpc_services,
    attachment_services=_attachment_rpc_services,
    background_services=_background_rpc_services,
    response_services=_response_rpc_services,
    approval_services=_approval_rpc_services,
)

_methods_session_lifecycle.register(
    _methods,
    services=_methods_session_lifecycle.SessionLifecycleServices(
        child_run_active=lambda session_id: _child_run_active(session_id),
        claim_or_reuse_live=lambda *args, **kwargs: _claim_or_reuse_live(*args, **kwargs),
        clear_session_context=lambda tokens: _clear_session_context(tokens),
        coerce_seed_history=lambda messages: _coerce_seed_history(messages),
        completion_cwd=lambda params: _methods_complete._completion_cwd(
            params,
            _completion_rpc_services,
        ),
        db_unavailable_error=lambda *args, **kwargs: _db_unavailable_error(*args, **kwargs),
        default_session_cwd=lambda: _default_session_cwd(),
        deferred_session_record=lambda *args, **kwargs: _deferred_session_record(*args, **kwargs),
        desktop_backend_contract=DESKTOP_BACKEND_CONTRACT,
        emit=lambda event, session_id, payload: _emit(event, session_id, payload),
        emit_session_info_for_session=lambda session_id, session: _emit_session_info_for_session(session_id, session),
        enable_gateway_prompts=lambda: _enable_gateway_prompts(),
        ensure_session_db_row=lambda session: _ensure_session_db_row(session),
        err=lambda rid, code, msg: _err(rid, code, msg),
        find_live_session_by_key=lambda session_key: _find_live_session_by_key(session_key),
        get_db=lambda: _get_db(),
        git_branch_for_cwd=lambda cwd: _git_branch_for_cwd(cwd),
        history_to_messages=lambda history: _history_to_messages(history),
        init_session=lambda *args, **kwargs: _init_session(*args, **kwargs),
        lazy_resume_info=lambda *args, **kwargs: _lazy_resume_info(*args, **kwargs),
        live_session_payload=lambda *args, **kwargs: _live_session_payload(*args, **kwargs),
        load_show_reasoning=lambda: _load_show_reasoning(),
        load_tool_progress_mode=lambda: _load_tool_progress_mode(),
        make_agent=lambda *args, **kwargs: _make_agent(*args, **kwargs),
        maybe_schedule_auto_continue=lambda sid, session, target: _maybe_schedule_auto_continue(sid, session, target),
        new_session_key=lambda: _new_session_key(),
        ok=lambda rid, result: _ok(rid, result),
        profile_configured_cwd=lambda profile_home: _profile_configured_cwd(profile_home),
        profile_db=lambda params=None: _profile_db(params),
        profile_home=lambda profile: _profile_home(profile),
        profile_scoped=lambda handler: _profile_scoped(handler),
        project_info_for_cwd=lambda cwd: _project_info_for_cwd(cwd),
        register_session_cwd=lambda session: _register_session_cwd(session),
        resolve_model=lambda: _resolve_model(),
        resolve_session_source=lambda source: _resolve_session_source(source),
        response_profile_name=lambda profile: _response_profile_name(profile),
        schedule_agent_build=lambda sid: _schedule_agent_build(sid),
        schedule_session_cap_enforcement=lambda: _schedule_session_cap_enforcement(),
        sess_nowait=lambda params, rid: _sess_nowait(params, rid),
        session_db=lambda session: _session_db(session),
        session_info=lambda agent, session: _session_info(agent, session),
        session_live_item=lambda sid, session, current: _session_live_item(sid, session, current),
        session_resume_lock=_session_resume_lock,
        sessions=lambda: _sessions,
        sessions_lock=_sessions_lock,
        set_session_context=lambda target: _set_session_context(target),
...
_pet_services = methods_pet.default_pet_services(emit=_emit)
methods_pet.register(
    _methods,
