    ...
    for attempt in range(max_stream_retries + 1):
        ...
        try:
            try:
                ...
                # end-of-stream marker so Relay can finalize the physical
                # attempt before Hermes returns. Do not drain an obsolete
                # guarded request after its watchdog has claimed cancellation.
                ...
                if event_stream.final_response is not None:
                    response_to_finish = event_stream.final_response
                else:
                    raise
            else:
            ...
            if callable(close_fn):
                try:
                    ...
                    # checked out of the httpx pool. Poison request-local
                    # clients so the caller cannot cache a leaked connection.
                    if client is not None:
                        agent._abort_request_openai_client(
                            active_client, reason="codex_stream_close_failed"
                        )
        return _finish_response(response_to_finish)
