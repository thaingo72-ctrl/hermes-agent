...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    def _publish_watchdog_timeout(error: TimeoutError) -> None:
        """Make a claimed Codex watchdog timeout authoritative.

        Once the request-local guard grants timeout ownership, any subsequent
        worker exception is only fallout from guard deactivation / transport
        abort and must not replace the retryable TimeoutError with an
        InterruptedError that the conversation loop interprets as a user stop.
        Legacy non-Codex callers retain the old first-result-wins behavior.
        """
        if _codex_request_guard is not None:
            result["response"] = None
            result["error"] = error
        elif result["error"] is None and result["response"] is None:
            result["error"] = error
    ...
    def _call():
        ...
        worker_error = None
        try:
            def _make_worker_client(reason: str, kind: str = "openai"):
                if agent.api_mode == "codex_responses":
                    assert _codex_request_guard is not None
                    if kind != "openai":
                        raise ValueError("guarded Codex requests require OpenAI clients")
            ...
            worker_response = _dispatch_nonstreaming_api_request(
                agent,
                api_kwargs,
        ...
        # Completion ownership is published only after request-local stream and
        # client cleanup. Until this point watchdogs and user interrupts must be
        # able to cancel a worker whose cleanup is blocked.
        if _codex_request_guard is not None and not _codex_request_guard.complete():
            return
        if worker_error is not None:
            result["error"] = worker_error
        else:
            result["response"] = worker_response
    ...
    while t.is_alive():
        ...
        if (
            ...
            t.join(timeout=0.1 if _codex_request_guard is not None else 2.0)
            if _silent_hint:
                _publish_watchdog_timeout(TimeoutError(
